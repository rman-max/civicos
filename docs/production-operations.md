...
3. Apply migrations `0001`–`0009` before application traffic. Railway API deployments run the checksum-tracked pre-deploy migration command automatically; other targets use `psql -v ON_ERROR_STOP=1`. Apply the St. Joseph County seed with the provisioner account separately.
