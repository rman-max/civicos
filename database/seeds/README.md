...
Seed files establish a tenant, its municipalities, departments, and approved public sources. They are configuration, not schema migrations: apply them only after all database migrations (currently through `0009`) and only with the privileged tenant-provisioning account described in `docs/autonomous-discovery.md`.
